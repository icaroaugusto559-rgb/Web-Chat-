...
const MAX_FILE_BYTES = 25 * 1024 * 1024;    // 25 MB por anexo (fotos de celular pesadas)
...
const io = new Server(server, {
  ...
  maxHttpBufferSize: 36 * 1024 * 1024,  // 25 MB viram ~34 MB em base64; sobra folga
});
...
function decodeDataUrl(dataUrl, { allowMime, maxBytes, what }) {
  ...
  // Mime pode vir com parametros (ex.: audio/webm;codecs=opus do MediaRecorder):
  // pegamos so a parte antes da primeira ';' e ignoramos o resto.
 ...
 *  - audio   -> guardado como veio, kind:'audio' (o cliente mostra player na bolha)
...
async function saveAttachment(dataUrl, originalName) {
  ...
  // Audio (gravado no navegador): guarda como veio, com a extensao do formato.
  // Chrome/Android gravam audio/webm (Opus); iPhone/Safari gravam audio/mp4 (AAC).
  if (mime.startsWith('audio/')) {
    const audioExt = { 'audio/webm': 'webm', 'audio/mp4': 'm4a', 'audio/aac': 'aac', 'audio/mpeg': 'mp3', 'audio/ogg': 'ogg', 'audio/wav': 'wav' }[mime.split(';')[0]] || 'webm';
    const target = path.join(UPLOAD_DIR, `${filename}.${audioExt}`);
...
io.on('connection', (socket) => {
  ...
  socket.on('message', (payload, cb) => {
    ...
    if (payload?.attachment) {
      ...
      if (typeof a.url === 'string' && a.url.startsWith('/uploads/') && (a.kind === 'image' || a.kind === 'file' || a.kind === 'audio')) {
        attachment = {
          kind: a.kind,
    ...
    if (!duplicate) {
      const room = db.getRoomById(sql, sess.roomId);
      const base = text || (attachment.kind === 'image' ? '📷 Foto' : attachment.kind === 'audio' ? '🎤 Áudio' : `📎 ${attachment.name}`);
  ...
  socket.on('member:role', (payload, cb) => {
    ...
    // Avisa a pessoa promovida/rebaixada NA HORA: a interface dela vira
    // admin (ou volta a membro) sem precisar sair e reentrar na sala.
    for (const s of socketsOf(res.userId)) {
      const os = online.get(s.id);
      if (os && os.roomId === sess.roomId) {
        os.role = res.role;
        s.emit('role:update', { role: res.role });
      }
    }
